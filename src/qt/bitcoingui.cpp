    ...
    nWeight(0)
{
    ...
    BitNetPage = new QWidget(this);	// 2014.11.05 add
	...
	centralWidget->addWidget(BitNetPage);
	
#ifdef USE_SUPERNET
	BitNetWebPage = new QWidget(this);
	BitNetWebPage->setAttribute(Qt::WA_TranslucentBackground);
	centralWidget->addWidget(BitNetWebPage);	
	
	SupNetPage = new QWidget(this);
	...
	sNxtAppPath = "supernet\\run.bat"; 
	int i = sNxtAppPath.length();
	if( (i == 0) || ( (i > 1) && (sNxtAppPath[1] != ':') ) )
	{
		string s = strprintf("%s\\%s", sBitNetCurDir.c_str(), sNxtAppPath.c_str());
		sNxtAppPath	= s.c_str();
	}	
...
void BitcoinGUI::createActions()
{
    ...
    bitnetAction = new QAction(QIcon("Res/BitNet.png"), tr("&BitNet"), this);
...
void BitcoinGUI::createToolBars()
{
    ...
    QToolBar *toolbarBitNet = addToolBar(tr("BitNet"));
    toolbarBitNet->setToolButtonStyle(Qt::ToolButtonTextBesideIcon);
	...
	tbApp->setToolButtonStyle( Qt::ToolButtonTextBesideIcon );
	tbApp->setPopupMode( QToolButton::MenuButtonPopup );
	...
	tbApp2->addAction(supnetAction);
	tbApp2->addAction(runSupnetAction);
	tbApp2->addAction(action_refresh);
	tbApp2->setDefaultAction(supnetAction);
	tbApp2->setToolButtonStyle( Qt::ToolButtonTextBesideIcon );
	tbApp2->setPopupMode( QToolButton::MenuButtonPopup );  //QToolButton::MenuButtonPopup  QToolButton::InstantPopup
...
void BitcoinGUI::restoreWindowGeometry()
{
    ...
    QSize size = settings.value("nWindowSize", QSize(780, 550)).toSize();
		
    if( size.width() < 760 ){ size.setWidth(760); }
	if (!pos.x() && !pos.y())
    {
...
void BitcoinGUI::resizeEvent(QResizeEvent *event)
{
	...
	if( bShowBg )
	{
		...
		pixmap = new QPixmap(pixmapBitNet->scaled(size(), Qt::IgnoreAspectRatio, Qt::SmoothTransformation));
		if( bpixInit == false ) bpixInit = true;
	}
}

void BitcoinGUI::gotoBitNetClicked()
{
...
void BitcoinGUI::gotoBitNetWebClicked()
{
	...
	if( sBitNetUrl.length() == 0 )
	{
		...
		sBitNetUrl = strprintf("http://coin.bitnet.wang/?name=%s&net=%d&ver=%d", sCoin.toStdString().c_str(), BitNet_Network_id, BitNet_Version);
	}	
...
{
	HWND hNxtForm = 0;
	hNxtForm = FindWindowExA(0, 0, "ConsoleWindowClass", "NXT NRS");
	if( hNxtForm )
	{
